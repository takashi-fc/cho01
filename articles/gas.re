...
があります。今回はGoogleDevelopersJapanで過去に紹介されたnode-google-apps-scriptを利用します。
 ...
 1. そのプロジェクトを開いて、[リソース] > [Googleの拡張サービス] を開いてDriveApiを有効にする
 1. そのまま[Google API コンソール]のリンクをクリック
 1. [Google Apps API] > [Drive API] > [有効にする]をクリック
 1. そのまま左メニューの[認証情報] > [認証情報を作成] > [OAuthクライアントID]をクリック
 1. アプリケーションの種類は[その他]、名前に[GoogleAppsScript-local-development]など適当な名前を設定し、作成
 1. OAuthクライアントという画面が表示されるがそのまま閉じる
 1. 作成した[GoogleAppsScript-local-development]の右側にあるダウンロードボタンをクリックし、jsonを保存
 1. @<tt>{npm install -g node-google-apps-script}
 1. @<tt>{gapps auth ダウンロードしたjsonのパス}
 1. 表示されるURLをブラウザで開いて、認証
 1. @<tt>{gapps init 1で作成したプロジェクトのID} (IDは[https://script.google.com/d/xxx/edit]の[xxx]部分)
 1. [gapps.config.json], [src/コード.js]が作成されている
...
さて、ローカルで快適にJavaScriptを書くことができるようになりましたが、今のままでは結局オンラインエディタ上で実行しないといけません。
 ...
 1. そのプロジェクトを開いて、[リソース] > [Googleの拡張サービス] を開いてDriveApiを有効にする
 1. そのまま[Google API コンソール]のリンクをクリック
 1. [Google Apps API] > [Google Apps Script Execution API] > [有効にする]をクリック
