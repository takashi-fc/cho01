...
みなさんGoogleAppsScriptは使ったことがありますか？@<br>{}
GoogleAppsScriptは簡単に言うと、Googleのサーバ上でJavaScriptを実行できるサービスです。@<br>{}
...
のようなことができます。ただ、オンラインエディタ上でしか書けないため、普段使いしている慣れた環境で開発することができません。@<br>{}
 ...
 * @<href>{https://github.com/danthareja/node-google-apps-script}
 * @<href>{https://github.com/soundTricker/gas-manager}
...
これでローカルで編集したファイルを反映させることができ、Gitで管理できるようになりました。@<br>{}
srcディレクトリでファイルを作成・削除・変更したものが同期されます。その際、.jsから.gsへとGoogleAppsScript用の形式に変換されます。.jsが不完全な場合に.gsへの変換が失敗し同期が失敗します。

== ES6で開発する

GoogleAppsScriptはES6に対応していないため、ES6で記述したJavaScriptをそのまま実行することはできません。@<br>{}
そのため@<href>{https://babeljs.io}を利用して、実行可能なJavaScriptのコードに変換します。@<br>{}
...
さて、ローカルで快適にJavaScriptを書くことができるようになりましたが、今のままでは結局オンラインエディタ上で実行しないといけません。@<br>{}
